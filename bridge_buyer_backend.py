...
# -------- Integrations (stubs) --------
@app.post("/ice-digital-trade", tags=["Integrations"], summary="ICE DT Stub", description="Stub endpoint to receive contract settlement payloads from ICE sandbox.", status_code=200)
async def ice_dt_stub(payload: dict):
    contract_id = payload.get("contract_id")
    external_ref = payload.get("external_ref")
    if contract_id:
        await database.execute("""
            UPDATE contracts SET status = COALESCE(status,'Pending'),
                                 signature = COALESCE(signature,'ICE'),
                                 signed_at = NOW()
            WHERE id = :id
        """, {"id": contract_id})
    return {"ok": True, "received": payload, "external_ref": external_ref, "note": "stub only"}
...
@app.get("/admin/export_all", tags=["Admin"], summary="Export all data (ZIP)", description="Exports contracts.csv and bols.csv in a ZIP.", status_code=200)
async def export_all():
    tmpdir = tempfile.TemporaryDirectory()
    base = pathlib.Path(tmpdir.name)

    async def dump_csv(sql, out):
    ...
    await dump_csv("SELECT * FROM contracts ORDER BY created_at DESC", "contracts.csv")
    await dump_csv("SELECT * FROM bols ORDER BY pickup_time DESC", "bols.csv")

    zip_path = base / "bridge_export.zip"
    with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as z:
        z.write(base / "contracts.csv", "contracts.csv")
        z.write(base / "bols.csv", "bols.csv")

    return FileResponse(str(zip_path), media_type="application/zip", filename="bridge_export.zip")
